...
pub fn routes() -> Router<PgPool> {
    Router::new()
        .route("/competitors", get(list).post(add))
...
async fn list(
    ...
    Ok(Json(
        json!({"items":items,"limit":50,"automatic_scans":false,"latch_research":false,"direct_reddit":false}),
    ))
}
async fn add(
    State(p): State<PgPool>,
    ...
    let project = v.project.trim();
    let website = v.website.trim();
    crate::domain::text(name, "Competitor name", 1, 100)?;
    if project.chars().count() > 120 {
        return Err(ApiError::invalid("Project must be at most 120 characters."));
    }
    if !website.is_empty() {
        let url = url::Url::parse(website)
            .map_err(|_| ApiError::invalid("Enter a full HTTPS website URL."))?;
        if website.len() > 1000
            || url.scheme() != "https"
            ...
            || url.password().is_some()
        {
            return Err(ApiError::invalid(
                "Enter a full HTTPS website URL without credentials.",
            ));
        }
    }
    ...
    let count: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM competitors WHERE workspace_id=$1 AND archived_at IS NULL",
    )
    .bind(&w.id)
    .fetch_one(&mut *tx)
    .await?;
    if count >= 50 {
        return Err(ApiError::conflict(
            "Archive a competitor before adding more than 50.",
        ));
    }
    ...
    if inserted == 0 {
        return Err(ApiError::conflict(
            "This competitor is already tracked for this project.",
        ));
    }
    tx.commit().await?;
...
async fn archive(
    ...
    if changed == 0 {
        return Err(ApiError::invalid("Competitor not found in this workspace."));
    }
...
async fn history(
    ...
    Ok(Json(
        json!({"items":rows,"limit":20,"coverage":"Up to five search results per run; not a complete market or Reddit census."}),
    ))
}
