...
async fn feed(
    ...
    tx.commit().await?;
    Ok(Json(
        json!({"control":control,"items":items,"mission":"Improve agent context quality and reduce token cost","capabilities":{"discovery":"tracked_agent_instructions","evaluation":"lossless_context_pack_v1","source_editing":false,"model_calls":false,"automatic_merge":false},"history_limit":100}),
    ))
}
async fn control(
    State(p): State<PgPool>,
