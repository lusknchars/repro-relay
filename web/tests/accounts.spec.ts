...
async function fixture(context: BrowserContext, mode: 'local' | 'team') {
  let signed=false, name='Test teammate', bio='', role=mode==='local'?'owner':'viewer'
  ...
  await context.route('**/api/v1/**',async route => {
    ...
    if(path==='/account/register') {signed=true;name=body.name;return route.fulfill({json:{authenticated:true,return_to:mode==='team'?'/?view=agents&case=RR-team-case':'/'}})}
    ...
    const result=path==='/session'?{mode,authenticated:mode==='local'||signed,role:signed?role:undefined}
      : path==='/account'?{enabled:true,authenticated:signed,bootstrap_available:mode==='local'&&!signed,shared:mode==='team',role:signed?role:null,profile:signed?{id:'ACC-fixture',username:'teammate',name,bio}:undefined}
