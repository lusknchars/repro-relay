...
test('case triage and investigation sections preserve context without starting an agent', async ({ page }) => {
  ...
  await page.goto('/')
  const desk = page.getByRole('region', { name: 'Investigation desk' })
  await expect(desk.getByRole('button', { name: 'Needs context: 1 case' })).toBeVisible()
  await expect(page.getByRole('navigation', { name: 'Workspace', exact: true }).getByRole('button', { name: 'Case inbox', exact: true })).toHaveAccessibleDescription('Triage reported problems')
  await page.screenshot({ path: 'test-results/investigation-desk.png', fullPage: true })
  await desk.getByRole('button', { name: 'Needs context: 1 case' }).focus()
  await page.keyboard.press('Enter')
  await expect(page).toHaveURL(/view=inbox/)
  await expect(page.getByLabel('Filter cases by status')).toHaveValue('needs_context')
