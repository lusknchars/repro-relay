...
import { ArrowRight, Check, ChevronRight, ClipboardList, FileText, History, Play, Plus, RefreshCw, Search, ShieldCheck, Square } from 'lucide-react'
...
function CaseInvestigation({ item, guest, onOpenCase, session, onRefresh }: { item: Case; guest: boolean; onOpenCase: (item: Case) => void; session: Session; onRefresh?: () => Promise<void> }) {
  ...
  function editDraft(patch: Partial<ReviewDraft>) { session.drafts.set(draftKey, { ...draft, ...patch }); setDraftVersion(version => version + 1) }
  ...
  function saveReview(event: React.FormEvent) {
    ...
    if (!current || !reviewable || pending || busy) return
  ...
  return <>
    <section className="iw-investigation" aria-label="Agent investigation">
      ...
      {current ? <>
        ...
        <div className="iw-section-heading"><h3>Agent findings</h3><span className="iw-tag">{latestReview ? reviewLabels[latestReview.decision] : 'Agent proposal'}</span></div>
        <p className="iw-subtle">{current.detail}</p>
        {current.output?.trim() ? <><div className="iw-proposal"><pre>{current.output}</pre></div><p className="iw-caption">Agent-generated proposal. A review records your decision; it does not verify a fix or publish memory.</p></> : <div className="iw-empty-panel"><ClipboardList aria-hidden="true" /><p>{isRunActive(current.status) ? 'Hermes has not returned findings yet. Its saved run progress appears below.' : 'This run did not return a proposal. Inspect its history before starting another investigation.'}</p></div>}
        ...
        <details className="iw-details" open><summary>Saved run progress <span>{current.events.length}</span></summary><p className="iw-caption">Coordinator state changes. Browser actions, screenshots, and repair verification are not collected by this adapter.</p><ol className="iw-timeline">{current.events.map(event => <li key={event.sequence}><span className="iw-timeline-mark" aria-hidden="true" /><div><p>{event.detail}</p><time dateTime={event.at}>{date(event.at)}</time></div></li>)}</ol></details>
        {reviewable && <form className="iw-review" onSubmit={saveReview}><div className="iw-section-heading"><h3>Review this proposal</h3><ShieldCheck aria-hidden="true" /></div><p className="iw-subtle">Tell Hermes what is useful and what it should check next. Your feedback stays attached to this run.</p><div className="iw-form-row"><label className="iw-field">Review decision<select value={draft.decision} disabled={!!busy || !!pending} onChange={event => editDraft({ decision: event.target.value as ReviewDraft['decision'] })}><option value="needs_changes">Needs another check</option><option value="accepted">Review accepted</option><option value="dismissed">Dismiss proposal</option></select></label><label className="iw-field">Reviewer name<input autoComplete="name" required maxLength={120} value={draft.reviewer} disabled={!!busy || !!pending} onChange={event => editDraft({ reviewer: event.target.value })} placeholder="Your name" /></label></div><label className="iw-field">Review feedback<textarea required maxLength={8000} rows={4} value={draft.feedback} disabled={!!busy || !!pending} onChange={event => editDraft({ feedback: event.target.value })} placeholder="Which claim needs evidence? What should Hermes check or correct?" /></label><div className="iw-actions"><Button type="submit" pending={busy === 'review'} disabled={!!busy || !!pending || !draft.reviewer.trim() || !draft.feedback.trim() || !!readError}><Check />Save review</Button><span className="iw-caption">Reviewer name is supplied locally.</span></div></form>}
    ...
    <aside className="iw-context" aria-label="Investigator context and controls">
      ...
      {current && <><div className="iw-section-heading"><h3>Reported usage</h3></div><dl className="iw-context-facts"><div><dt>Total tokens</dt><dd>{current.usage?.total_tokens?.toLocaleString() ?? 'Not reported'}</dd></div><div><dt>Cost</dt><dd>{current.usage?.cost_usd == null ? 'Not reported' : `$${current.usage.cost_usd.toLocaleString(undefined, { maximumFractionDigits: 6 })}`}</dd></div><div><dt>Time limit</dt><dd>{current.max_seconds} seconds</dd></div></dl><details className="iw-details iw-packet"><summary>Context used by selected run</summary><p className="iw-caption">Build {current.build} · Revision {current.case_revision}</p><pre>{JSON.stringify(current.context, null, 2)}</pre></details></>}
    </aside>
  </>
}
