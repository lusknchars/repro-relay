...
export function RunPanel({item, guest}: {item:Case; guest:boolean}) {
  ...
  async function update(work:()=>Promise<unknown>) {
    if(refreshRequested.current)return
    refreshRequested.current=true;setBusy(true);setError('')
    try {await work();setRuns(await request<InvestigationRun[]>(`/cases/${item.id}/runs`))}
    catch(e){setError(message(e))}
    finally{setBusy(false);refreshRequested.current=false}
  }
  ...
  return <section className="run-panel" aria-label="Agent investigation">
    <div className="run-heading"><div><h3>Agent investigation</h3><p>{guest?'Guest workspaces do not operate a connected agent.':runner?.reason || 'Checking the runner connection…'}</p></div>
      {!guest&&<Button variant="ghost" size="sm" disabled={busy} onClick={()=>void update(async()=>setRunner(await request('/runner')))}><RefreshCw/>Check connection</Button>}
    </div>
    {!guest&&<div className="run-controls"><label>Time limit<select aria-label="Investigation time limit" value={maxSeconds} disabled={busy||active(latest)} onChange={e=>setMaxSeconds(Number(e.target.value))}><option value={30}>30 seconds</option><option value={120}>2 minutes</option><option value={300}>5 minutes</option><option value={600}>10 minutes</option></select></label>
      <Button disabled={busy||!runner?.available||active(latest)||!item.build.trim()} onClick={()=>void update(start)}><Play/>Start Hermes investigation</Button>
      {!item.build.trim()&&<p>Name the current build before starting.</p>}
    </div>}
    {latest&&<div className="run-current">
      ...
      <div className="run-controls">
        {active(latest)&&!guest&&<Button variant="outline" size="sm" disabled={busy||latest.status==='stopping'} onClick={()=>void update(()=>request(`/runs/${latest.id}/stop`,{method:'POST'}))}><Square/>Request stop</Button>}
        {latest.status==='attention'&&!guest&&<Button variant="outline" size="sm" disabled={busy} onClick={()=>void update(()=>request(`/runs/${latest.id}/reconcile`,{method:'POST'}))}><RefreshCw/>Reconcile run</Button>}
      </div>
