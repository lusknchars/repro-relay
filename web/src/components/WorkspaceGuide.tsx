...
export function WorkspaceGuide({ navigate, browserTools, guest, connections = false }: Props) {
  ...
  return <section className="my-5 rounded-xl border bg-card p-5 sm:p-6" aria-label={connections ? 'Agent tool connections' : 'Workspace guide'}>
    ...
    ].map(step => <div className="flex flex-col gap-3 rounded-lg border p-4" key={step.title}><h3 className="text-sm font-medium">{step.title}</h3><p className="text-muted-foreground flex-1 text-xs leading-5">{step.body}</p><Button className="min-h-11 justify-start" variant="ghost" onClick={() => navigate(step.view)}>{step.action}<ArrowRight/></Button></div>)}</div>}
    {connections && <div className="mt-5 grid gap-5 lg:grid-cols-2"><section className="rounded-lg border p-4"><h3 className="flex items-center gap-2 text-sm font-medium"><Terminal className="size-4"/>Terminal tools</h3><p className="text-muted-foreground my-3 text-xs leading-5">Run this once from the Repro Relay repository to connect the installed Codex CLI. It checks the local API and adds project-scoped MCP settings. Global settings and existing project configuration stay intact; no agent is launched.</p><code className="block break-all rounded bg-muted p-3 text-xs">{terminalCommand}</code><Button className="mt-3 min-h-11" variant="outline" disabled={guest} onClick={() => void copy(terminalCommand, 'terminal')} >{copied === 'terminal' ? <Check/> : <Copy/>}{copied === 'terminal' ? 'Copied' : 'Copy setup command'}</Button><p className="text-muted-foreground mt-3 text-xs">Terminal session control and code execution are not connected by these evidence tools.</p></section><section className="rounded-lg border p-4"><h3 className="flex items-center gap-2 text-sm font-medium"><Globe className="size-4"/>WebMCP browser tools</h3><p className="my-3 text-xs" role="status">{status[browserTools]}</p><p className="text-muted-foreground text-xs leading-5">A compatible browser agent can inspect Relay directly. Browser support is detected at runtime; the regular interface remains available.</p><ul className="mt-3 space-y-2">{manifest.map(t => <li key={t.name} className="text-xs"><code className="break-all">{t.name}</code></li>)}</ul><p className="text-muted-foreground mt-3 text-xs">Tools cannot approve, edit code, change policy, or send messages.</p></section></div>}
