...
export function ToolLibrary({ guest, readOnly, navigate, openPlow, browserTools }: {
  ...
  return <section className="tool-library my-5 space-y-6" aria-label="Agent tools library">
    ...
    <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border bg-card p-4"><div><h3 className="text-sm font-medium">Know what each connection costs</h3><p className="mt-1 max-w-3xl text-xs leading-5 text-muted-foreground">Hermes reports are available in Agent controls. Pi usage stays in its terminal session. Mem0 and Moonshot billing stay with those services; a missing amount is not zero.</p></div><Button variant="ghost" className="min-h-11" onClick={() => navigate('agents')}>Inspect Hermes usage<ChevronRight/></Button></div>
