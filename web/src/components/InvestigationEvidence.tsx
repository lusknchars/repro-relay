...
function EnvironmentDetails({ value }: { value?: Environment }) {
  ...
  return <p className="iw-caption">{captureLabels[value.capture_mode] || value.capture_mode} · {value.name} · {value.os} {value.os_version} · {value.browser} {value.browser_version} · {value.device}{value.emulated ? ' · Emulated' : ''}</p>
}

export function InvestigationEvidence({ runId, runVersion, localValidation }: { runId: string; runVersion: number; localValidation: boolean }) {
  ...
  async function load(section: Section, after: number | null, signal: AbortSignal) {
    ...
    try {
      const page = await readEvidence<Page>(`/runs/${encodeURIComponent(runId)}/${section}?limit=20${after == null ? '' : `&after=${after}`}`, signal)
  ...
  useEffect(() => {
    ...
    setOpened(null); setOpening(''); setArtifactError(''); artifactRequest.current++
    setPages({ journal: initial(), artifacts: initial(), findings: initial() })
    ...
    // All requests belong to this run/version and are discarded on selection change.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [runId, runVersion, generation])

  useEffect(() => { if (opened) inspector.current?.focus() }, [opened])
  ...
  return <section className="iw-evidence" aria-label="Investigation evidence">
    ...
    {(['findings', 'journal', 'artifacts'] as const).map(section => {
      ...
      return <details key={section} className="iw-details" open><summary>{titles[section]} <span>{page.items.length}{page.next_cursor == null ? '' : '+'}</span></summary>
        ...
        {page.items.map(item => <article key={item.id} id={`evidence-${item.id}`} className="iw-review-record ie-record">
          {section === 'findings' ? <>
    ...
    {opened && <section className="ie-inspector iw-details" aria-label="Stored artifact contents" tabIndex={-1} ref={inspector}>
      <div className="iw-section-heading"><h3>{opened.name}</h3><Button variant="ghost" size="sm" onClick={() => { setOpened(null); artifactTrigger.current?.focus() }}>Close log</Button></div>
