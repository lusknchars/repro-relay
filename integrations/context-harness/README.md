...
Every non-empty valid instruction snapshot produces an evaluation candidate, including snapshots with no repeated content. Previously recorded unique snapshots acquire a candidate on the next monitor cycle. Existing approval and decline decisions are preserved. Empty instruction inventories remain audit-only. Identical whole-file detection remains a storage metric, not a prerequisite for reviewing context or proof that guidance is optimal or conflict-free.

Automatic evaluation builds a candidate representation with one body per unique content hash and a source-path mapping. The API independently reconstructs the expected representation and rejects altered or missing source content. It records actual serialized input/candidate bytes, including mapping overhead. A candidate can be larger. Storage reduction is not token savings, model quality, or an applied prompt optimization. No file in the checkout changes. Once the result is recorded, the user approves or declines the candidate in Harness → Repository context. Approval makes the exact current bundle available at `GET /autonomy/context`. The [Pi adapter](../pi-harness/README.md) can explicitly inspect or review these contents. It does not automatically activate them as skills or execute their instructions.
