...
**Hardened after the final review.** Commit `b091e56` changed code that Tasks 3 and 4 above show in its earlier form, so the committed module is authoritative where they differ. The alert goes only to an `https` base and a chat id matching `[A-Za-z0-9_-]{1,180}`, through `plow_opener()`, which ignores proxy settings and refuses redirects. The service runs `python3 -I -S`. `load_state` refuses symlinks, FIFOs, files over 1 MiB and deep nesting. An alert that raises is logged as `owner alert failed with <Type>`. `settings()` and `main(configured, sleep, log)` are testable, and `main` stands down without `PLOW_AGENT_TOKEN` or `PLOW_HOME_CHANNEL`. The CI step imports the module under `-I -S`, and `agent/README.md` documents the service. The suite is 48 watchdog tests, 56 in all.
