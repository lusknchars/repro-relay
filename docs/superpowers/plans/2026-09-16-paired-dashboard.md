...
Verify statically; do not ask the running agent and do not rebuild or restart it. `agent/Dockerfile` bakes skills in with `COPY skills/ /opt/hermes/skills/` and no volume mounts the repository's skills directory, so a file added to the repository is invisible to the live agent until the image is rebuilt, and asking it to list its skills would fail regardless of whether this file is correct.

Confirm instead that the file exists at `agent/skills/relay-pair/SKILL.md`; that its front matter matches `agent/skills/relay-reach/SKILL.md` and `agent/skills/video-digest/SKILL.md`; that every route it names is registered in `crates/relay-api/src/pairing.rs` or `crates/relay-api/src/conversations.rs`; and that a character scan of its prose finds no hyphen or dash.
