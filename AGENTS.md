...
GitHub access follows `docs/GITHUB-ACCESS.md`: agents and the app hold no GitHub credential, read GitHub only through the person's own `gh` login on their own machine, and never write to it. `make check` fails when a GitHub token appears in a tracked file.
